...
import { fetchGetPagingList, fetchDelete } from '@/service/api/industria';
import { industriaApiInterfaceTypeRecord, taskStatusRecord } from '@/constants/options';
...
const { columns, data, loading, getData, getDataByPage, mobilePagination, searchParams, resetSearchParams } = useTable({
  ...
  columns: () => [
    ...
    },
    {
      ...
      title: $t('page.industriaApi.interfaceName'),
      align: 'center',
      minWidth: 120
    },
    {
      ...
      title: $t('page.industriaApi.interfaceType'),
      align: 'center',
      minWidth: 120,
      customRender: ({ record }) => {
        ...
        const label = $t(industriaApiInterfaceTypeRecord[record.interfaceType]);
        return label;
      }
    },
    {
      ...
      title: $t('page.deviceConnection.deviceName'),
      minWidth: 120
    },
    {
      key: 'status',
      dataIndex: 'status',
      title: $t('page.industriaApi.status'),
      align: 'center',
      minWidth: 120,
      customRender: ({ record })=> {
...
<template>
  <div class="min-h-500px flex-col-stretch gap-16px overflow-hidden lt-sm:overflow-auto">
    ...
    <a-card :title="$t('page.industriaApi.title')" :bordered="false" class="card-wrapper sm:flex-1-hidden">
      <template #extra>
