...
import { paramTypeOptions } from '@/constants/token';
    ...
    <a-table :data-source="formModel.rows" :columns="columns" :pagination="false" :scroll="{ y: 400 }" row-key="key" class="editable-table">
      <template #bodyCell="{ column, record, index }">
        <template v-if="column.dataIndex === 'name'">
          <a-form-item :name="['rows', index, 'name']" no-style :rules="[{ required: true }]" :validate-trigger="['change', 'blur']" style="margin-bottom: 0">
        ...
        <template v-else-if="column.dataIndex === 'valueType'">
          <a-form-item :name="['rows', index, 'valueType']" no-style :rules="[{ required: true }]" validate-trigger="change" style="margin-bottom: 0">
        ...
        <template v-else-if="column.dataIndex === 'value'">
          <a-form-item :name="['rows', index, 'value']" no-style :rules="[{ required: true }]" :validate-trigger="['change', 'blur']" style="margin-bottom: 0">
            <a-input v-model:value="record.value" :placeholder="$t('page.httpRequest.form.paramValue')" />
          </a-form-item>
        </template>
        <template v-else-if="column.dataIndex === 'description'">
          <a-form-item :name="['rows', index, 'description']" style="margin-bottom: 0">
