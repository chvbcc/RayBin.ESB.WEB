...
const dateRange = computed({
  get: () => {
    if (model.value.startCreateTime && model.value.endCreateTime) {
      return [model.value.startCreateTime, model.value.endCreateTime] as RangeValue;
    }
    return undefined;
  },
  set: (value) => {
    if (value && value.length === 2) {
      model.value.startCreateTime = value[0];
      model.value.endCreateTime = value[1];
    } else {
